...
public class WildCard {
    /*
    와일드카드 문법 (<?>)
    제너릭 타입에서 구체적인 타입대신 ?를 사용하는 방법 - 주로 메서드에서 사용된다.
    ...
    public static void printAnyBox(Box2 <?> box){
        ...
        // 타입 안전성이 깨질 수 있어 set(쓰기)는 문법적으로 막아두었다.

        // 매개변수로 Box2<String, Integer> 타입의 객체를 넘겨주더라도
        // 다운캐스팅이 불가능하다. -> Object로 취급가능.
        // String myData = box.getItem();
    ...
    public static void printAnimalBox(Box2<? extends Animal> animalBox) {
        ...
        Animal animal = animalBox.getItem(); // 읽기가능
        // set(쓰기)는 불가능하다. - 타입 안정성이 깨질수 있어서 불가능.
    }

    // 하한 경계 와일드카드
    // Object까지 업캐스팅을 할것이다. 다운캐스팅을 Animal까지 제한
