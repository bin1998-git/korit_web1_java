...
public class Main {
    public static void main(String[] args) {
        ...
        boolean nineStudent = students.stream()
                .anyMatch(socre -> socre.getScore() >= 90);
        System.out.println(nineStudent);
        ...
        // 3. "컴퓨터공학"과 이면서 90잠 이상인 학생이름을 담은 리스트 출력 // 정렬x
        // 필터림 -> 객체에서 문자열변환 -> 모으기
        List<String> results = students.stream()
                .filter(student
                        -> student.getMajor().equals("컴퓨터공학") && student.getScore() >= 90)
        ...
        // distinct -> 객체상대로 distinct()를 적용하면, equals(), hashcode()로비교
