...
public class OperationMain {
    public static void main(String[] args) {
        ...
        // 결과가 모두 true면 true
        System.out.println(isAllOver2Name);

        boolean noShortName = names.stream()
                .noneMatch(name -> name.length() < 2);
        // 결과가 모두 false면 true
        System.out.println(noShortName);
        ...
        String longestName = names.stream()
                ...
                .orElse("없음"); // names가 비어있을 수 있음 그러면 없음을 리턴해라
    ...
    //추가) Optional?
    // 변수에 값이 존재할 수도있고, 없을 수도 있는 상황에서 사용
    // 변수를 필드로 가지는 컨테이너 클래스 -> null로 인한 예외를 사전에 예방하려고 사용

    // 5. reduce
    // numbers에 있는 모든값을 총합을 구해보자!
